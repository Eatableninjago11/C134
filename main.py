...
fig = px.scatter(x=planet_radiuses, y=planet_masses, color=planet_type)
fig.show()
...
for planet_data in suitable_planets:
  if planet_data[9].split(" ")[1].lower() == "days":
    ...
    planet_data[9] = float(planet_data[9].split(" ")[0]) #365 #year
...
planet_speeds = []
for planet_type in suitable_planets:
  distance = 2 * 3.14 * (planet_type[8] * 1.496e+9)
  time = planet_type[9] * 86400
  speed = distance / time
...
for index, planet_type in enumerate(temp_speed_supporting_planets):
  if planet_speeds[index] > 200:
    speed_supporting_planets.remove(planet_type)
...
for index, planet_data in enumerate(planet_data_rows):
  ...
  try:
    if planet_data[6].lower() == "terrestial" or planet_data[6].lower() == "super earth":
      features_list.append("planet type")
  except: pass
